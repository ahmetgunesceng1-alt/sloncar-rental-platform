...
def test_profile_happy_path():
    ...
    assert response["status"] == 200 and response["body"] is not None
...
def test_update_profile_invalid_auth():
    response = make_request("PATCH", "/api/auth/profile", body={
        "name": "Updated Test Bot"
    }, headers={"Authorization": "Bearer invalid-token-12345"})
    print_req_res("PATCH", "/api/auth/profile", response)
...
def run_tests():
    ...
    setup_auth()
    tests = [
        test_register_happy_path, test_register_not_found, test_register_invalid_auth,
        test_login_happy_path, test_login_not_found, test_login_invalid_auth,
        test_profile_happy_path, test_profile_not_found, test_profile_invalid_auth,
        test_update_profile_happy_path, test_update_profile_not_found, test_update_profile_invalid_auth
    ]
    for test in tests:
        try:
            test()
